...
| 12 | **No pairs at all — six single plies (2026-09-16)** | The inner plies went the way the two-ply optics did: a bare inner ply bought nothing but wall thickness, and bonding twelve plies by hand is a step that can only go wrong. Every face is ONE written 2.25 mm ply, butt-jointed; interior 27.5 × 27.5 × 30.5 mm (4.5 mm of ring clearance). Foil is **1/4″** (fold 2.05 mm; 3/8″ would fold onto the 3.64 mm art rim). The mask did not change: the die sizes were already the single-ply cut dims, and the two numbers the bonded formulas used to derive — the art rim 3637.5 µm and the ID-tick band 2943.75 µm — are pinned in `production.py` as the values the 15 September plate was written with. Consequence to decide: at the 1/4″ fold the ID ticks sit OUTSIDE the foil, in the clear rim, visible unless the solder bead covers them; moving them is a mask change. Code: `bonded` removed end to end, `ply_cuts.face_rects`, golden fixture regenerated without the bonded cases. Bench procedure: `docs/assembly.md`. |
...
`app/witness_dies.py` — `build_face_die` (ONE ply per face from
`export_fine.build_plate_fine`'s single-ply branch, mirrored, optionally
rotated 90°, ID + dicing ticks, finished and inverted per die; it refuses a
two-ply spec). `production_cells()` sizes every die from the same blank solve
`export_blank` uses: box 32 × 32 × 35 mm, outer plies 32 × 32 (lid) /
32 × 30.5 (front) / 27.5 × 30.5 (sides). Eleven written plies: lid ×2, front ×2
(one rotated), the six photographs and a spare beach side.

`app/region_art.py` — the single-layer centrepiece contract (decision 10): a
motif returns an int label raster over the art box and a `Region(name,
period_um, duty)` per label; `export_fine._emit_region_art` writes every
region as a vertical 50 % grating at its period (or solid gold at period 0)
on its own 20 µm art-box raster with a one-cell glass gutter between regions,
and the preview (`plates._paste_centerpiece`, `_literal_period_raster`) and the
fab SVG (`single_layer_region_rects`) read the same map. The lid registers
`monogram-jp` (per-letter regions, `motifs/monogram.py::monogram_regions`), the
front the new `artistic/globe_atlantic.py` (`geo/render.py::globe_regions`).

`export_witness.layout` is a **dicing grid** (decision 11): rows of one
height, straight 1 mm streets, EDGE cuts (every cut runs along a die edge with
the blade in the street, two cuts per street, so dies come out at drawn size
with any blade under 0.5 mm), the cut list in `layout()["dicing"]`,
`write_dicing_md` → `DICING.md` next to the mask, red dashes on the map SVG,
GDS layer 2/0, and 0.5 mm clear SAW-LANE MARKS in the chrome at the ends of
every cut line (`_dice_edge_marks`; `tools/dev/add_dice_marks.py` re-stamps a
written plate). The bench cells are the single-layer essentials — M-POL, M-CD
dense/iso, M-DUTY 10/5, D-PER, WEDGE 44, H-ACU — in two rows under the dies.
**19 cells**, 111.8 of 117.9 mm of height.

The photographs' colour is **authored per picture**
(`assets/photos/<image>.colour.json`, `photo.colour_plan("authored", image)`,
`tools/dev/render_colour_plans.py` to re-render the set); the sides carry no
carrier (`photo.CARRIER_COV` = 0), the picture dissolves to bare glass inside a
garland of per-family diffractive leaf gratings (decision 5b).
...
| A2 regions | `validate_dies.py regions` | the lid's and the front's region gratings AS WRITTEN (the die's own polygons, rastered at 0.5 µm): metal fraction and stripe pitch per region against the map's duty and period; false-colour picture of each art box. Results: see §7 |
...
| Geometry (written) | `witness_dies.clear_field` (decompose, then the tiled checker on the pieces) + `tools/dev/probe_gds_sanity.py` on the GDS | 2026-09-15 plate: the CLEAR data as written, merged, Euclidian, zero-distance touches excluded: lid 0 / 0 (both copies), base none, sides 0 / 0 except garden 2 / 0 and porch 0 / 3, front 76 / 1 (both copies; 0.6–1.9 µm slivers at region seams) — **~160 sub-2 µm spots on the plate**, resist-unresolvable, listed per die. 1.93 M shapes, max 22 vertices, no zero-area, data inside the field. Build 459 s (sides 43–93 s each, lid 10 s, front 31 s, base 0 s), write 20 s; GDS 121.5 MB, OASIS 3.9 MB |
...
Read M-POL, M-CD, M-DUTY, H-WEDGE, D-PER under a lamp. Dice per `DICING.md`
(decision 11): 2.25 mm fused quartz on 1 mm streets is a saw job — the first
plate was cleaved by hand and broke. The die edge is the box's cut dimension,
so the kerf stays inside the street and the edge is held to ±50 µm. Bond each
written outer ply to a bare 2.25 mm inner ply (sized one ply smaller per edge,
from the second, uncoated blank) chrome-down; there is nothing to register, so
the bond is a squaring job, not an alignment. The back and bottom are bare
glass and are not on this plate.
