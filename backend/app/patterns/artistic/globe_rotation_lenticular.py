...
# Silhouette quality floor: below ~192 rows the globe's graticule and coastlines
# break up, and the two spin states stop reading as different rotations. Handed
# to barrier_lattice, which trades it off against the litho floor.
MIN_GRID_ROWS = 192
...
class GlobeRotationStereo(Pattern):
    ...
    params = [
        ...
        # Slit bounds are set by the litho floor, not by taste: the front comb's
        # narrowest feature is period·min(duty, 1-duty) and the back interlace
        # cell is period/cols_per_period (≤ period/8 for any extent in range),
        # so period ≥ 20 µm with duty in [0.2, 0.8] keeps every emitted feature
        # — line AND gap, on BOTH layers — at or above the litho floor. The old
        # 8 µm / 0.1 corner advertised a 0.8 µm slit on a 2 µm process.
        ...
        return (
            {
                ...
                # Honest because the registration is enforced: the slit
                # straddles a channel boundary at every extent, so the switch
                # completes at a back shift of p/4 and the clean first zone ends
                # at p/2. Shared Snell helper — see
                # patterns._helpers.exterior_tilt_deg.
    ...
    def generate(
        ...
        check_lattice_budget(
            n_grid * n_stripes,
            "globe-rotation-stereo interlace",
            slit_period_um=slit_period_um,
            extent_um=extent_um,
        )
        ...
        # (straddle class) with channel A (-Δ/2) on its +x side — the whole
        # effect lives there.
        #
        ...
        # Rasterize the two spin states onto the slit-derived pixel grid so each
        ...
        # Each slit period gets one half per spin state: left half = -Δ/2 view,
        # right half = +Δ/2 view. cols_per_period is even by construction, so
        # the two channels are exactly p/2 wide.
        ...
        # the B half, a -x shift the A half — which is the class description's
        # "-tilt channel sees -Δ/2, +tilt channel sees +Δ/2". (The
        # pre-convergence comment here had the two signs the wrong way round.)
        #   Scene A = globe at -Δ/2 → -tilt
        #   Scene B = globe at +Δ/2 → +tilt
