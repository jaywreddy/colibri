...
export type CollageMetrics = {
  /** mean |dL| over mean luminance for the most-different pair of tiles. */
  effect_strength: number;
  /** Michelson swing of the per-tile MEANS — what a shimmer moves. */
  mean_swing: number;
  /** fraction of pixels that actually change between that pair. */
  changed_frac: number;
  /** the two angles that differ most, in degrees. */
  peak_pair_deg: number[];
  tile_means: number[];
  /** the axis the sweep actually used. */
  axis: 'x' | 'y';
  /** what was requested ('auto' means the backend chose). */
  axis_mode: string;
};

export type CollageSheet = {
  /** object URL for the PNG — REVOKE it when replacing or unmounting. */
  url: string;
  metrics: CollageMetrics;
};

export type CollageOpts = {
  spanDeg?: number;
  steps?: number;
  illum?: string;
  axis?: 'auto' | 'x' | 'y';
  tilePx?: number;
  cols?: number;
  /** draw the slug/illum caption inside the PNG (off when the UI shows it). */
  showTitle?: boolean;
  signal?: AbortSignal;
};

/**
 * One tilt-sweep sheet for a pattern, plus the numbers for the same sweep.
 *
 * The metrics ride along on a response header rather than coming from a second
 * call, because recomputing them means compositing the whole sweep again — and
 * this host must never run two heavy computes at once.
 */
export async function fetchCollage(
  ...
  const q = new URLSearchParams({
    span_deg: String(opts.spanDeg ?? 6),
    steps: String(opts.steps ?? 9),
    illum: opts.illum ?? 'ambient',
    axis: opts.axis ?? 'auto',
    tile_px: String(opts.tilePx ?? 150),
  });
  if (opts.cols !== undefined) q.set('cols', String(opts.cols));
  if (opts.showTitle === false) q.set('show_title', 'false');
  const r = await tracedFetch(`/sim/collage/${slug}/default?${q}`, {
    signal: opts.signal,
  });
  if (!r.ok) throw new Error(await errorDetail(r, `Collage ${slug}`));
  const raw = r.headers.get('X-Collage-Metrics');
  if (!raw) throw new Error(`Collage ${slug}: response carried no metrics`);
  const blob = await r.blob();
  return { url: URL.createObjectURL(blob), metrics: JSON.parse(raw) as CollageMetrics };
}
